...
{
    public async Task<List<ActivityDto>> Handle(GetActivitiesByCustomerIdQuery request, CancellationToken ct)
    {
        var activities = await repo.FindAsync(a => a.CustomerId == request.CustomerId);
